...
function Update-RegistryProfilePaths {
    ...
    foreach ($sub in Get-ChildItem $regPath) {
        ...
        if ($user -in @('Default', 'Public')) {
            $expectedPath = Join-Path $newRoot $user
            if ($p.ProfileImagePath -ne $expectedPath) {
                Set-ItemProperty -Path $sub.PSPath -Name "ProfileImagePath" -Value $expectedPath
                Write-Output "Updated registry for $user -> $expectedPath"
                $regUpdated.Value += $user
            } else {
                Write-Output "Registry for $user already correctly set, skipping update."
            }
        }
        elseif (($user -in $copiedProfiles) -and ($user -notin $excludedProfiles)) {
