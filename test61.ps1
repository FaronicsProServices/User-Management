...
function Get-ProfilesNotOnPDrive {
    ...
    foreach ($sub in Get-ChildItem $regPath) {
        ...
        if ($p.ProfileImagePath -and ($p.ProfileImagePath -notlike "P:\*")) {
            if ($user -notin $GLOBALS:excludedProfiles) {
...
function Copy-Profiles {
    ...
    foreach ($folder in Get-ChildItem $sourceRoot -Directory -Force) {
        ...
        if ($folder.Name -in $GLOBALS:excludedProfiles) { continue }
...
function Update-RegistryProfilePaths {
    ...
    foreach ($sub in Get-ChildItem $regPath) {
        $p = Get-ItemProperty $sub.PSPath
        $user = Split-Path $p.ProfileImagePath -Leaf
        if (($user -in $copiedProfiles -or $user -in @('Default', 'Public')) -and ($user -notin $GLOBALS:excludedProfiles)) {
