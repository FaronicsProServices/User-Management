...
function Copy-Profiles {
    param($sourceRoot, $destRoot, [ref]$skipped, [ref]$copied, [ref]$alreadyExists)
    ...
    foreach ($folder in Get-ChildItem $sourceRoot -Directory -Force) {
        ...
        # Check if profile already exists on destination
        if (Test-Path $dst) {
...
function Retry-CopySkippedProfiles {
    param($sourceRoot, $destRoot, [string[]]$skippedUsers, [ref]$newlyCopied, [ref]$stillSkipped)

    foreach ($user in $skippedUsers) {
        $src = Join-Path $sourceRoot $user
        $dst = Join-Path $destRoot $user

        if (-not (Test-Path $src)) {
            Write-Warning "Skipped profile source $user does not exist, skipping."
            continue
        }
        ...
        if ($isLoaded) {
            Write-Warning "Skipped profile $user still in use, skipping copy again."
            $stillSkipped.Value += $user
            continue
        }

        Write-Output "Retry copying skipped profile: $user"
        robocopy $src $dst /MIR /COPYALL /R:3 /W:5 /XJ | Out-Null
        if (Test-Path $dst) {
            $newlyCopied.Value += $user
        } else {
            Write-Warning "Failed to copy skipped profile $user on retry."
        }
    }
}
...
# Copy other profiles with in-use skip detection and check for already existing profiles
$skipped = @()
$copied = @()
$alreadyExists = @()
Copy-Profiles -sourceRoot $profilesRoot -destRoot $destRoot -skipped ([ref]$skipped) -copied ([ref]$copied) -alreadyExists ([ref]$alreadyExists)
...
if ($skipped.Count -gt 0) {
    ...
    # Retry copying skipped profiles now
    $retryCopied = @()
    $stillSkipped = @()
    Retry-CopySkippedProfiles -sourceRoot $profilesRoot -destRoot $destRoot -skippedUsers $skipped -newlyCopied ([ref]$retryCopied) -stillSkipped ([ref]$stillSkipped)

    if ($retryCopied.Count -gt 0) {
        Write-Output "Profiles copied on retry: $($retryCopied -join ', ')"
        $copied += $retryCopied
    }
    if ($stillSkipped.Count -gt 0) {
        Write-Warning "Profiles still skipped after retry: $($stillSkipped -join ', ')"
    }
}
