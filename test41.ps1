...
function Update-RegistryProfilePaths {
    ...
    # Default & Public profiles updated in correct base registry key, not ProfileList subkeys
    $baseReg = "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion"

    $newDefault = Join-Path $newRoot "Default"
    Set-ItemProperty -Path $baseReg -Name "DefaultUserProfile" -Value $newDefault
    Write-Output "Updated Default profile path -> $newDefault"
    $regUpdated.Value += "Default"

    $newPublic = Join-Path $newRoot "Public"
    Set-ItemProperty -Path $baseReg -Name "PublicProfile" -Value $newPublic
    Write-Output "Updated Public profile path -> $newPublic"
    $regUpdated.Value += "Public"

    # ProfilesDirectory (important for future profiles)
    Set-ItemProperty -Path $baseReg -Name "ProfilesDirectory" -Value $newRoot
...
if ($notUpdated.Count -eq 0) {
    Write-Output "Registry paths updated successfully for all profiles."
} else {
