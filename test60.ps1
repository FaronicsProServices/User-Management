...
# ===== System Profiles Exclusion =====
$excludedProfiles = @('systemprofile', 'LocalService', 'NetworkService')
...
function Get-ProfileFolders {
    ...
    Get-ChildItem -Path $root -Directory -Force | Where-Object {
        $_.Name -notin @('All Users', 'Default', 'Public') -and $_.Attributes -notmatch 'ReparsePoint' -and $_.Name -notin $excludedProfiles
    }
}
...
function Get-ProfilesNotOnPDrive {
    param($newRoot)
    ...
    foreach ($sub in Get-ChildItem $regPath) {
        ...
        if ($p.ProfileImagePath -and ($p.ProfileImagePath -notlike "P:\*")) {
            if ($user -notin $excludedProfiles) {
                $toMigrate += [PSCustomObject]@{Name=$user; RegPath=$p.ProfileImagePath}
            }
        }
    }
    return $toMigrate
...
function Copy-Profiles {
    ...
    foreach ($folder in Get-ChildItem $sourceRoot -Directory -Force) {
        ...
        if ($folder.Name -in $excludedProfiles) { continue }
        if ($null -ne $profilesToCopy -and ($folder.Name -notin $profilesToCopy)) {
            continue
        }
        Write-Output "Checking profile $($folder.Name) for copy eligibility..."
        $src = $folder.FullName
...
function Update-RegistryProfilePaths {
    ...
    foreach ($sub in Get-ChildItem $regPath) {
        $p = Get-ItemProperty $sub.PSPath
        $user = Split-Path $p.ProfileImagePath -Leaf
        if (($user -in $copiedProfiles -or $user -in @('Default', 'Public')) -and ($user -notin $excludedProfiles)) {
            ...
            Write-Output "Skipped registry update for $user (profile not copied or excluded)."
        }
    }
...
$profilesToCopy = @(Get-ProfilesNotOnPDrive -newRoot $destRoot | ForEach-Object { $_.Name })
Write-Output "Profiles with old registry path requiring copy: $($profilesToCopy -join ', ')"

$skipped = @()
$copied = @()
$alreadyExists = @()

Copy-Profiles -sourceRoot $profilesRoot -destRoot $destRoot -skipped ([ref]$skipped) -copied ([ref]$copied) -alreadyExists ([ref]$alreadyExists) -profilesToCopy $profilesToCopy
